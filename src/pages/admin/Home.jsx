...
import { GET_PASLON_FOR_ADMIN, DELETE_PASLON_DATA, SUBSCRIBE_PASLON } from "../../apollo/Paslon"
...
function HomePage() {
  ...
  const [ deletePaslon, {loading: loadingDelete} ] = useMutation(DELETE_PASLON_DATA, {
                ...
                <div className="rounded-lg shadow-xl bg-white max-w-sm">
                  ...
                  <div className="p-6" key={paslon.id}>
                    <span className="py-1 uppercase text-gray-500 font-bold">
                      total voting <span className="ml-2">{paslon.total_voted}</span>
                    </span>
                    <div className="flex mt-3">
                      <Link to='/Admin/UpdateCandidate' state={{ 
                          id: paslon.id, 
                          visi: paslon.visi, 
                          misi: paslon.misi, 
                        ...
                        <button type="button" className="inline-block rounded-full bg-blue-300 leading-normal shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out w-9 h-9">
                          <img src={updateIcon} className="m-auto" />
                        </button>
                      </Link>
                      <button onClick={() => handleDelete(paslon.imageFileName, paslon.id)} type="button" className="inline-block rounded-full bg-red-300 leading-normal shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out w-9 h-9 ml-4">
